...
## 🏛️ CERTUM PROJECT OVERRIDES (CHECK FIRST)

**When working on Certum project, ALWAYS read `.agent/rules/certum-overrides.md` FIRST.**

These rules **OVERRIDE** general guidelines below:

| Override       | Kit Rule          | Certum Rule                        |
| -------------- | ----------------- | ---------------------------------- |
| **Colors**     | Blue Trap Ban     | ✅ Trust Blue `#4A9CC9` allowed    |
| **Background** | No specification  | ✅ `#FFFFFF` mandatory             |
| **Glass**      | Glass Trap Ban    | ✅ OK with `border-black/5`        |
| **Gradients**  | Mesh Gradient Ban | ✅ OK with opacity ≤ 0.3           |
| **Stack**      | Next.js default   | ✅ Vite + React                    |
| **Motion**     | Generic GSAP      | ✅ Lenis `lerp: 0.05` + power2.out |

**Mandatory Components:**

- `<KineticHeading />` — All section titles
- `<SiteAtmosphere />` — Global texture
- `<ScrambleText />` — Text reveals
- `<NumberTicker />` — Animated stats

> 🔴 **Before any Certum work:** Read `.agent/rules/certum-overrides.md`

> 📚 **Library Documentation:** `.agent/rules/library-docs-index.md` — Index of GSAP, Tailwind, Framer Motion, and other library docs in `.antigravity/knowledge/`
