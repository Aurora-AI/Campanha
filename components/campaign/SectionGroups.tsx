...
export default function SectionGroups() {
  ...
  return (
    <section className="w-full py-36 bg-stone-50">
      <div className="mx-auto w-[min(1400px,92vw)]">
         <FadeIn>
            <div className="flex flex-col md:flex-row justify-between items-end mb-16 border-b border-stone-200 pb-8">
                ...
                <div className="flex items-center gap-4 mt-4 md:mt-0">
                    ...
                    <div className={`px-4 py-2 rounded-full text-white text-xs font-bold uppercase tracking-widest ${statusColorMap[status as keyof typeof statusColorMap] || 'bg-gray-400'}`}>
                        {statusLabel}
                    </div>
                </div>
            </div>
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {groupsRadial.map((group, index) => (
                <FadeIn key={group.group} delay={index * 0.1}>
                    <motion.div
                        className="bg-white p-8 rounded-sm shadow-sm flex flex-col items-center cursor-default"
                        whileHover={{
                            y: -8,
         ...
         <FadeIn delay={0.4}>
             <motion.div
                className="mt-16 bg-white border border-stone-200 p-8 flex flex-col md:flex-row items-center justify-between gap-6"
